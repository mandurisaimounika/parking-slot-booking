...
import { getUUID, isValidUUID } from "./getUuid";
import authenticateToken from "./authentication";
import checkPermission from "./checkPermissions";
import { bookingsByIdValidationMiddleware } from "./joi-validation/joi-get-booking-by-id";
import { bookingsByIdAndSpoitValidationMiddleware } from "./joi-validation/joi-put-booking-by-id";
import { createBookingsValidationMiddleware } from "./joi-validation/joi-post-new-booking";
import { executeQuery, dataQueryWithId } from "./database/dataQuery";
...
route.get(
  ...
  async (req: ExtendedRequest, res) => {
    ...
    if (booking[0] && isValidUUID(id)) {
      if (
        req.user.role === "admin" ||
        booking[0].createdby === req.user.firstname
      ) {
        res.json(booking);
      } else {
        res.status(403).json({
...
route.post(
  ...
  async (req: ExtendedRequest, res) => {
    ...
    const text =
      "INSERT INTO bookings (id, createdby, starttime, endtime, parkingspot, createdat) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *";
  ...
  async (req: ExtendedRequest, res) => {
    ...
    if (booking[0] && isValidUUID(id)) {
      if (
        ...
        const text =
          "UPDATE bookings SET parkingspot = $1, updatedat = $2 WHERE id = $3 RETURNING *";
